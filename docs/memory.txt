...
4. malloc和free
◼ malloc 和 free 是 C/C++ 标准库中用于动态内存管理的函数. malloc 用于在堆上分配指定大小的内存块，并返回指向该内存块的指针；
free 用于释放之前通过 malloc 分配的内存块
◼ 在不同的内存需求下，malloc 采用不同的系统调用来分配内存:
    ● 小于 128KB: 通常使用 brk/sbrk 系统调用扩展数据段(heap)，在已有的堆空间中管理内存块。
    ● 大于 128KB: 直接使用 mmap 系统调用映射一块新的虚拟地址空间，以满足大块内存的需求。
    ● brk: 用于设置数据段(heap)的末端地址(program break)，即扩展或收缩堆空间。
    ● sbrk: 用于相对移动 program break，通常用于在堆上分配连续的内存块。
◼ mmap 系统调用用于将文件或匿名内存映射到进程的虚拟地址空间，常用于分配大块内存或内存映射文件。
    ● 在 malloc 分配内存时，通常在用户得到的指针前面留出一段空间，用于存放该内存块的元数据
    ● 物理地址映射: 通过页表将虚拟地址映射到物理内存页面(在首次访问时通过缺页中断)
    ● 操作系统通过页表和 MMU 将虚拟地址映射到物理地址。
